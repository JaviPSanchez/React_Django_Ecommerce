...
# Cada usuario va a tener una lista de ordenes y va a poder visualizar su lista

# Enlistar las ordenes del usuario:
class ListOrdersView(APIView):
    def get(self, request, format=None):
        user = self.request.user
        try:
            orders = Order.objects.order_by('-date_issued').filter(user=user)
            # Mapeamos el resultado de las ordenes
            result = []
...
class ListOrderDetailView(APIView):
    # Aqui pasamos el id de cada orden
